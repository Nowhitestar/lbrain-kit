...
3. Use `scripts/operations.py` operation `skill.preview`. Supply every explicit runtime target at this stage. It validates the proposed package and runtime plan, recommends the next semantic version, writes the exact diff and preview hash to the Proposal, and does not modify the canonical Skill. Never ask the user to run the script or a CLI.
4. Show the exact Change Preview, including tests, version rationale, and runtime destinations. Ask for one explicit approval of that immutable preview. Any preview, Skill-baseline, runtime-target, or runtime-state change invalidates the approval.
5. After approval, record the matching Proposal as `accepted` with the exact preview hash, then use operation `skill.apply` with that returned preview and hash. Apply accepts no additional runtime target and rejects a pending Proposal; never reproduce the diff manually or edit a runtime copy as the source of truth.
...
7. Root- and runtime-scoped transaction locks serialize apply through validation and rollback. A failure restores the canonical Skill, version, tests, and any changed runtime target; a rollback conflict is reported rather than overwriting a concurrent edit. Keep the Proposal accepted but not applied with safe failure evidence. A stale baseline invalidates application and requires a new preview and decision.
