...
3. Before any local write, use the extension confirmation UI to show the extracted title and whether the save will contain article text, a Thread, selection, original file, video link/subtitles, or a generic HTML snapshot. The toolbar popup appears before extraction finishes, immediately shows saving after confirmation, and restores permission, saving, or terminal state when reopened. Continue only after explicit confirmation; cancellation has no write or Native Host side effect. Request only temporary exact-origin access when a confirmed capture needs a cross-origin image, document, subtitle, or audio attachment, then remove only the permission added by that capture after the attempt.
